...
if (!baseUrl || !username || !password) {
    console.error('Missing RNA_API_BASE_URL, RNA_API_USERNAME or RNA_API_PASSWORD environment variables. ' +
...
async function getKpisFor(region, sessionDate, normalWorkDayMinutes) {
    const [routes, equipmentTypes] = await Promise.all([
        client.getRoutes(region, sessionDate, true),
        client.getEquipmentTypes(),
    ]);
    const equipmentById = new Map(equipmentTypes.map((e) => [e.identity?.identifier ?? e.id, e]));
...
server.registerTool('get_routes', {
    ...
    description: 'Lightweight list of planned routes for a region and date from Roadnet Anywhere: route id, description, start time and stop count. Read-only.',
    inputSchema: {
...
server.registerTool('get_route_stops', {
    ...
    const route = routes.find((r) => r.identity?.identifier === routeId);
    if (!route) {
        return {
...
server.registerTool('get_route_kpis', {
    title: 'Get route KPIs',
    description: 'Compute KPIs (duration, weight utilization, plain-language insights) for every route on a given region and date. Read-only.',
    inputSchema: {
        region: z.string().describe('RNA region identifier or code'),
        sessionDate: z.string().describe('Date in yyyy-MM-dd format'),
...
server.registerTool('get_dashboard_summary', {
    ...
    description: 'Aggregate KPI summary across all routes for a region and date, with a plain-language recommendation. Read-only.',
    inputSchema: {
        region: z.string().describe('RNA region identifier or code'),
        sessionDate: z.string().describe('Date in yyyy-MM-dd format'),
        normalWorkDayMinutes: z
